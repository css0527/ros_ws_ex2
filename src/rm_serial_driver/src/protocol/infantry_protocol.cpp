...
/*************send*************/
/******************************/
//自瞄发送   //帧头：0xff 帧尾：0x0d 数据长度：16字节
void ProtocolInfantry::send(const auto_aim_interfaces::msg::GimbalCmd &data) {
  packet_.loadData<unsigned char>(0xec, 1); //1
  // std::cout <<"pitch:  "  << data.pitch << "\n";
  // std::cout <<"yaw:    "  << data.yaw << "\n";
  packet_.loadData<unsigned char>(data.fire_advice ? FireState::Fire : FireState::NotFire, 2);//2
  packet_.loadData<float>(static_cast<float>(data.pitch), 3); //3-6
  packet_.loadData<float>(static_cast<float>(data.yaw), 7);//7-10
  packet_.loadData<float>(static_cast<float>(data.distance), 11);//11-14
  // // chassis control
  // // linear x
...
bool ProtocolInfantry::receive(auto_aim_interfaces::msg::SerialReceiveData &data) {
  ...
  if (packet_tool_->recvPacket(packet)) {
    ...
    packet.unloadData(enemy_color, 1); //1
